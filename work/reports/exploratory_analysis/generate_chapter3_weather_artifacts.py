...
# ==============================================================================
# FIGURE 3.7: Multi-Hazard Historical Shock Signatures (5 Archetypes)
# ==============================================================================
def generate_figure_3_7():
    print("Generating Figure 3.7 (Multi-Hazard Historical Shock Footprints)...")
    
    archetype_keys = [1988, 1983, 1974, 1993, 1994]
    if 2012 in years_available:
        archetype_keys.append(2012)
        
    shock_data = {}
    for yr in archetype_keys:
        if yr in years_available:
            fpath = os.path.join(WEATHER_DIR, f"county_daily_{yr}.parquet")
            df_yr = pd.read_parquet(fpath, columns=[
                "date", "air_temperature_maximum", "total_precipitation",
                "volumetric_soil_water_layer_2", "volumetric_soil_water_layer_3", "vapor_pressure_deficit"
            ])
            df_mean = df_yr.groupby("date").mean().reset_index()
            df_mean["doy"] = df_mean["date"].dt.dayofyear
            df_season = df_mean[(df_mean["doy"] >= 91) & (df_mean["doy"] <= 304)].copy()
            df_season["cum_precip"] = np.cumsum(df_season["total_precipitation"])
            df_season["rootzone_sm"] = 0.3 * df_season["volumetric_soil_water_layer_2"] + 0.7 * df_season["volumetric_soil_water_layer_3"]
            shock_data[yr] = df_season
            
    climatology = []
    for yr in years_available:
        fpath = os.path.join(WEATHER_DIR, f"county_daily_{yr}.parquet")
        df_yr = pd.read_parquet(fpath, columns=["date", "total_precipitation", "volumetric_soil_water_layer_2", "volumetric_soil_water_layer_3", "air_temperature_maximum"])
        df_m = df_yr.groupby("date").mean().reset_index()
        df_m["doy"] = df_m["date"].dt.dayofyear
        df_s = df_m[(df_m["doy"] >= 91) & (df_m["doy"] <= 304)].copy()
        df_s["cum_precip"] = np.cumsum(df_s["total_precipitation"])
        df_s["rootzone_sm"] = 0.3 * df_s["volumetric_soil_water_layer_2"] + 0.7 * df_s["volumetric_soil_water_layer_3"]
        climatology.append(df_s)
        
    df_clim = pd.concat(climatology, ignore_index=True).groupby("doy").mean().reset_index()
    ref_dates = pd.to_datetime("2021-01-01") + pd.to_timedelta(df_clim["doy"] - 1, unit="D")
    
    fig, axes = plt.subplots(3, 1, figsize=(11, 10.5), sharex=True)
    
    shock_meta = {
        1988: {"label": "1988: Summer Heatwave & Severe Drought (-25.6%)", "color": "#d95f02", "ls": "-", "lw": 2.2},
        1983: {"label": "1983: Late-Summer Pod-Fill Drought (-21.6%)", "color": "#e66101", "ls": "--", "lw": 1.8},
        1974: {"label": "1974: Wet Sowing Delay & Frost Shock (-19.7%)", "color": "#8073ac", "ls": "-.", "lw": 1.8},
        1993: {"label": "1993: Great Midwest Inundation / Flood (-10.5%)", "color": "#2b83ba", "ls": "-", "lw": 2.2},
        1994: {"label": "1994: Historic Optimum Bumper Harvest (+14.3%)", "color": "#1b9e77", "ls": "-", "lw": 2.2},
        2012: {"label": "2012: Historic Flash Drought (-11.7%)", "color": "#e7298a", "ls": "--", "lw": 2.0}
    }
    
    # Panel (a): Cumulative Precipitation
    ax0 = axes[0]
    ax0.plot(ref_dates, df_clim["cum_precip"], color="black", lw=2.5, ls=":", label="Historical Baseline Climatology")
    for yr in [1993, 1994, 1988, 1983, 1974]:
        if yr in shock_data:
            m = shock_meta[yr]
            ax0.plot(ref_dates[:len(shock_data[yr])], shock_data[yr]["cum_precip"], color=m["color"], lw=m["lw"], ls=m["ls"], label=m["label"])
    ax0.set_ylabel("Cumulative Rain (mm)")
    ax0.set_title("(a) Cumulative Growing Season Precipitation: 1993 Flood Surplus vs. 1988/1983 Deficits", loc="left", fontweight="bold")
    ax0.legend(loc="upper left", frameon=True, fontsize=9)
    
    # Panel (b): Root-Zone Soil Moisture
    ax1 = axes[1]
    ax1.plot(ref_dates, df_clim["rootzone_sm"], color="black", lw=2.5, ls=":", label="Historical Baseline Climatology")
    for yr in [1993, 1994, 1988, 1983, 1974]:
        if yr in shock_data:
            m = shock_meta[yr]
            ax1.plot(ref_dates[:len(shock_data[yr])], shock_data[yr]["rootzone_sm"], color=m["color"], lw=m["lw"], ls=m["ls"])
    ax1.set_ylabel(r"Soil Moisture ($\text{m}^3/\text{m}^3$)")
    ax1.set_title(r"(b) Integrated Root-Zone Soil Water Dynamics (Layers 2 & 3, 7--100 cm)", loc="left", fontweight="bold")
    ax1.set_ylim(0.15, 0.44)
    
    # Panel (c): Maximum Temperature 15-day rolling
    ax2 = axes[2]
    roll_clim_tmax = df_clim["air_temperature_maximum"].rolling(15, center=True).mean()
    ax2.plot(ref_dates, roll_clim_tmax, color="black", lw=2.5, ls=":", label="Historical Climatology")
    for yr in [1993, 1994, 1988, 1983, 1974]:
        if yr in shock_data:
            m = shock_meta[yr]
            roll_tmax = shock_data[yr]["air_temperature_maximum"].rolling(15, center=True).mean()
            ax2.plot(ref_dates[:len(roll_tmax)], roll_tmax, color=m["color"], lw=m["lw"], ls=m["ls"])
    ax2.axhline(30, color="#b2182b", ls=":", lw=1.2, alpha=0.7)
    ax2.text(ref_dates[5], 30.5, r"Critical Reproductive Stress Threshold ($30^\circ$C)", color="#b2182b", fontsize=8.5, fontweight="bold")
    ax2.set_ylabel(r"15-Day $T_{\text{max}}$ ($^\circ$C)")
    ax2.set_xlabel("Date across Growing Cycle")
    ax2.set_title("(c) Thermal Dynamics: 15-Day Rolling Maximum Temperature ($T_{\text{max}}$)", loc="left", fontweight="bold")
    ax2.set_ylim(10, 36)
    
    # Highlight July-August flowering & pod-fill
    jul_start = pd.to_datetime("2021-07-01")
    aug_end = pd.to_datetime("2021-08-31")
    for ax in axes:
        ax.axvspan(jul_start, aug_end, color="#fb8072", alpha=0.12, zorder=0)
    ax0.text(pd.to_datetime("2021-08-01"), ax0.get_ylim()[1]*0.9, "Critical Flowering & Pod-Fill (Jul--Aug)", ha="center", fontsize=9.5, fontweight="bold", color="#b2182b")
    ...
    plt.close()
    print(f"Figure 3.7 (Shock Footprints) saved to {pdf_path}")


# ==============================================================================
# FIGURE 3.8: Phenological Climate-Yield Sensitivity / Correlations
# ==============================================================================
def generate_figure_3_8():
    print("Generating Figure 3.8 (Monthly Climate-Yield Correlations)...")
    ...
    for yr in years_available:
        ...
        df_yr = pd.read_parquet(fpath, columns=[
            "county_fips", "date", "air_temperature_maximum", "total_precipitation",
            "vapor_pressure_deficit", "volumetric_soil_water_layer_2", "volumetric_soil_water_layer_3"
        ])
        df_yr["year"] = yr
        df_yr["month"] = df_yr["date"].dt.month
        df_yr = df_yr[(df_yr["month"] >= 4) & (df_yr["month"] <= 10)].copy()
        df_yr["rootzone_sm"] = 0.3 * df_yr["volumetric_soil_water_layer_2"] + 0.7 * df_yr["volumetric_soil_water_layer_3"]
    ...
    plt.savefig(png_path)
    plt.close()
    print(f"Figure 3.8 (Sensitivity) saved to {pdf_path}")
